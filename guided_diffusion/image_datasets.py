...
def load_data(
    *,
    ...
    """
    if not data_dir and not dataset_split_file:
        raise ValueError("unspecified data directory")
    if dataset_split_file is None:
        all_files = _list_image_files_recursively(data_dir)
    else:
        with open(dataset_split_file, 'r') as data_subset:
            all_files = [os.path.join(data_dir, line.strip()) for line in data_subset]
    classes = None
    if class_cond:
