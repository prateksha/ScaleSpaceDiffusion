...
def _map_experiment_type(experiment_type):
    et = str(experiment_type).lower()
    if et == "ddpm":
        return ExperimentType.STANDARD
    raise ValueError(f"GaussianDiffusion only supports experiment_type 'ddpm', got '{experiment_type}'")
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        logger.log(
            f"[LOGGING] gaussian_diffusion/GaussianDiffusion_init:\n"
        ...
        self.experiment_type = _map_experiment_type(experiment_type)
        ...
        if inference_checkpoint is not None:
            self.inference_checkpoint = str(inference_checkpoint)
            logger.log(f"In gaussian diffusion: setting path for inference: {self.inference_viz_path}/{self.inference_checkpoint}")
    ...
    def q_posterior_mean_variance(self, x_start, x_t, t):
        ...
        if dist.get_rank() == 0 and self.dump_visualization_interval>0:
            ...
            self._log_single_image_inference(( _extract_into_tensor(self.posterior_mean_coef1, t, x_t.shape) * x_start)[:INFERENCE_VISUALIZATION_NUM].detach().cpu(), self.inference_checkpoint, t[0].cpu(), "x0_contribution" )
    ...
    def p_mean_variance(
        ...
        cond_rs = None
        model_output = model(x, self._scale_timesteps(t), None, **model_kwargs)
        if dist.get_rank() == 0 and self.dump_visualization_interval>0:
    ...
    def p_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            logger.log(
                "p_sample_loop (progressive):\n"
                f"  exp type      : {self.experiment_type}\n"
                f"  exp name      : {self.experiment_name}\n"
                f"  exp root_dir  : {self.experiment_root_dir}"
            )           
            if self.experiment_type == ExperimentType.STANDARD:
                img = th.randn(*shape, device=device)
            else:
                raise NotImplementedError(f"experiment type: {self.experiment_type}")
        indices = list(range(self.num_timesteps))[::-1]
        logger.log(f"sampling for indices= {indices}")
    ...
    def ddim_sample_loop(
        ...
        **kwargs
        ...
        """
        if len(kwargs)>0:
            logger.log(f"got kwargs! {kwargs}")
        final = None
    ...
    def training_losses(self, model, x_start, t, r, model_kwargs=None, noise=None, step=None):
        ...
        if self.experiment_type == ExperimentType.STANDARD:
            return self._training_losses_standard(model, x_start, t, model_kwargs, noise, step)
        else:
            raise NotImplementedError(f"Unknown experiment type: {self.experiment_type}")
    ...
    def _training_losses_standard(self, model, x_start, t, model_kwargs=None, noise=None, step=None):
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, self._scale_timesteps(t), None, **model_kwargs)
            ...
            _pred_x0_from_eps = self._predict_xstart_from_eps(x_t=x_t, t=t, eps=model_output)
            if dist.get_rank() == 0 and self.dump_visualization_interval>0 and step % self.dump_visualization_interval == 0:
                ...
                self._log_single_image(step, t[0], _pred_x0_from_eps[:TRAINING_VISUALIZATION_NUM].detach().cpu(), "pred_x0_from_eps" )
            dist.barrier()
    ...
    def _log_single_image(self, step, timestep_val, batch_images, tag):
        ...
        # logger.log(f"Dumping images: dir={dir_path}, file={filepath}")
    ...
    def _log_single_image_inference(self, images, inference_checkpoint, timestep_val, tag):
        ...
        dir_path = f"{self.inference_viz_path}/{inference_checkpoint}/{timestep_val:03d}"
        filepath = f"{dir_path}/{tag}_{timestep_val}_{images[0].shape}.png"
        os.makedirs(dir_path, exist_ok=True)
        torchvision.utils.save_image(image_grid, filepath)
